...
export function LoginForm({
  ...
  const [pass, setPass]:[string, Dispatch<SetStateAction<string>>] = useState("");
  const [hidden, setHidden]:[boolean, Dispatch<SetStateAction<boolean>>] = useState(true)
  ...
  const [err, setErr]:[AuthApiError, Dispatch<SetStateAction<AuthApiError>>] = useState();
  ...
  return (
    ...
    <div className={cn("flex flex-col gap-6 ", className)} {...props}>
      <Card className="bg-[#5c4033f0] border-0 text-white">
        <CardHeader>
          <CardTitle className="uppercase pb-1 text-center text-lg">Welcome</CardTitle>
          <CardTitle className="uppercase pb-2 text-center text-lg">KB's Kustomz Private Dashboard</CardTitle>
    ...
    description={
      <div className="text-left"><br></br>{err?.message} . . . please try again
        ...
        <div className="text-xs text-gray-500 text-left">
          <br></br>Supabase Code [{err?.code}]
          <br></br>Status Code [{err?.status}]
        </div>
      </div>}
