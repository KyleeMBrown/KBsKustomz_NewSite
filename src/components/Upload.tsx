...
export const Upload = ({imageCount, uploadImage}:{imageCount:number, uploadImage:Function}) => {
  ...
  const handleUpload = async (): Promise<void> => {
    try {
      ...
      // Compress images before requesting to upload
      const compressedFiles:File[] = await compressFiles(filesToUpload);
     ...
     // const imageCount:number = await getImageCount();
      ...
      // upload each file
      await Promise.all(
        compressedFiles.map(async (file: File, index: number) => {
          try {
            // upload using uploadImage server action
            await uploadImage(file, index, imageCount);
          } catch (e) {
            // handle errors
